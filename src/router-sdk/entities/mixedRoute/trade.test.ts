...
describe('MixedRouteTrade', () => {
  ...
  describe('is backwards compatible with pure v3 routes', () => {
            ...
            [
              {
                ...
                route: new MixedRouteSDK(
                  [pool_0_1, pool_weth_1],
                  token0,
                  ETHER
                ),
              },
    ...
    describe('#priceImpact', () => {
      describe('100% v3 route', () => {
        describe('tradeType = EXACT_INPUT', () => {
          ...
          it('is cached', () => {
            expect(exactIn.priceImpact === exactIn.priceImpact).toStrictEqual(
              true
            );
          });
          it('is correct', () => {
    ...
    describe('#bestTradeExactIn', () => {
      ...
      it('provides best route', async () => {
        ...
        expect(
          result[0].outputAmount.equalTo(
            CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(9971))
          )
        ).toBeTruthy();
        ...
        expect(
          result[1].outputAmount.equalTo(
            CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(7004))
          )
        ).toBeTruthy();
      });
      ...
      it('works for ETHER currency input', async () => {
        ...
        expect(result[0].swaps[0].route.path).toEqual([
          WETH9[1],
          token0,
          token1,
          token3,
        ...
        expect(result[1].swaps[0].route.path).toEqual([
          WETH9[1],
          token0,
          token3,
        ]);
    ...
    describe('#minimumAmountOut', () => {
      describe('tradeType = EXACT_INPUT', () => {
        ...
        it('throws if less than 0', () => {
          expect(() =>
            exactIn.minimumAmountOut(new Percent(JSBI.BigInt(-1), 100))
          ).toThrow('SLIPPAGE_TOLERANCE');
        });

        it('returns exact if 0', () => {
          expect(
            exactIn.minimumAmountOut(new Percent(JSBI.BigInt(0), 10000))
          ).toEqual(exactIn.outputAmount);
        });

        it('returns exact if nonzero', () => {
          expect(
            exactIn.minimumAmountOut(new Percent(JSBI.BigInt(0), 100))
          ).toEqual(CurrencyAmount.fromRawAmount(token2, 7004));
          expect(
            exactIn.minimumAmountOut(new Percent(JSBI.BigInt(5), 100))
          ).toEqual(CurrencyAmount.fromRawAmount(token2, 6670));
          expect(
            exactIn.minimumAmountOut(new Percent(JSBI.BigInt(200), 100))
          ).toEqual(CurrencyAmount.fromRawAmount(token2, 2334));
        });
      });
    });
  });
  ...
  describe('multihop v2 + v3', () => {
    ...
    describe('#fromRoutes', () => {
      ...
      it('can be constructed with ETHER as output for exact input with multiple routes', async () => {
        const trade = await MixedRouteTrade.fromRoutes<
          Token,
          Ether,
          TradeType.EXACT_INPUT
        >(
