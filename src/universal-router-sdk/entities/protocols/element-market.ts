...
export class ElementTrade extends NFTTrade<ElementData> {
  ...
  encode(planner: RoutePlanner, config: TradeConfig): void {
    for (const item of this.orders) {
      if (item.order.erc20Token.toLowerCase() != ElementTrade.ETH_ADDRESS)
        throw new Error('Only ETH supported');
      if (
        item.order.taker != ZERO_ADDRESS &&
        item.recipient.toLowerCase() != item.order.taker.toLowerCase()
      )
        throw new Error('Order has fixed taker');

      const value = this.getOrderPriceIncludingFees(item.order);

      const calldata = ElementTrade.INTERFACE.encodeFunctionData(
        'buyERC721Ex',
        [
          item.order,
          item.signature,
