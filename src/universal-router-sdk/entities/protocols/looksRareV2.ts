...
export class LooksRareV2Trade extends NFTTrade<LooksRareV2Data> {
  ...
  encode(planner: RoutePlanner, config: TradeConfig): void {
    const { takerBids, makerOrders, makerSignatures, totalValue, merkleTrees } =
      this.refactorAPIData(this.orders);

    let calldata;
    if (this.orders.length == 1) {
      calldata = LooksRareV2Trade.INTERFACE.encodeFunctionData(
        'executeTakerBid',
        [
          takerBids[0],
          makerOrders[0],
  ...
  getBuyItems(): BuyItem[] {
    ...
    for (const item of this.orders) {
      ...
      const tokenType =
        item.apiOrder.collectionType == LooksRareV2Trade.ERC721_ORDER
          ? TokenType.ERC721
          : TokenType.ERC1155;
