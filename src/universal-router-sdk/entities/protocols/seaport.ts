...
export class SeaportTrade extends NFTTrade<SeaportData> {
  ...
  public static OPENSEA_CONDUIT_KEY: string =
  ...
  encode(planner: RoutePlanner, config: TradeConfig): void {
    for (const order of this.orders) {
      ...
      let orderFulfillments: FulfillmentComponent[][] = order.items.map(
        (_, index) => [{ orderIndex: index, itemIndex: 0 }]
      );
      ...
      if (advancedOrders.length == 1) {
        calldata = SeaportTrade.INTERFACE.encodeFunctionData(
  ...
  getTotalOrderPrice(
    ...
    for (const item of order.items) {
      totalOrderPrice = totalOrderPrice.add(
        this.calculateValue(item.parameters.consideration, token)
      );
    }
    ...
    return considerations.reduce(
      (amt: BigNumber, consideration: ConsiderationItem) =>
        consideration.token.toLowerCase() == token.toLowerCase()
          ? amt.add(consideration.startAmount)
          : amt,
      BigNumber.from(0)
    );
  }
