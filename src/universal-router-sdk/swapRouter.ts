...
import { MethodParameters } from '../v3-sdk';
import { Trade as RouterTrade } from '../router-sdk';
import { Currency, TradeType } from '../sdk-core';
...
export abstract class SwapRouter {
  ...
  public static swapCallParameters(
    trades: Command[] | Command,
    config: SwapRouterConfig = {}
  ): MethodParameters {
    if (!Array.isArray(trades)) trades = [trades];

    const nftTrades = trades.filter((trade, _, []) =>
      trade.hasOwnProperty('market')
    ) as SupportedNFTTrade[];
    const allowRevert =
      // @ts-ignore
      nftTrades.length == 1 && nftTrades[0].orders.length == 1 ? false : true;
      ...
      if (trade.tradeType == RouterTradeType.NFTTrade) {
        ...
        if (currentNativeValueInRouter.lt(tradePrice)) {
          transactionValue = transactionValue.add(
            tradePrice.sub(currentNativeValueInRouter)
          );
          currentNativeValueInRouter = BigNumber.from(0);
        } else {
          currentNativeValueInRouter =
            currentNativeValueInRouter.sub(tradePrice);
        }
        /**
         * is UniswapTrade
         */
        ...
        const outputIsNative =
          uniswapTrade.trade.outputAmount.currency.isNative;
        ...
        if (inputIsNative) {
          transactionValue = transactionValue.add(
            BigNumber.from(
              uniswapTrade.trade
                .maximumAmountIn(swapOptions.slippageTolerance)
                .quotient.toString()
            )
          );
        }
        ...
        if (outputIsNative && swapOptions.recipient == ROUTER_AS_RECIPIENT) {
          currentNativeValueInRouter = currentNativeValueInRouter.add(
            BigNumber.from(
              uniswapTrade.trade
                .minimumAmountOut(swapOptions.slippageTolerance)
                .quotient.toString()
            )
          );
        }
  ...
  public static swapNFTCallParameters(
    ...
    const allowRevert =
      // @ts-ignore
      trades.length == 1 && trades[0].orders.length == 1 ? false : true;
